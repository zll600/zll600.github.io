...
export const DIR_VITEPRESS = resolve(
...
export async function listPages(
  dir: string,
  options: { target?: string; ignore?: string[] },
) {
  const { target = "", ignore = [] } = options;
 ...
 * @returns 路由树
 */
function addRouteItemRecursion(
  indexes: ArticleTree[],
  item: ArticleTree,
  path: string[],
  upgradeIndex: boolean,
) {
  if (path.length === 1) {
    indexes.push(item);
    return indexes;
    ...
    if (path.length === 1 && path[0] === "index") {
      ...
      // 否则，递归遍历
      obj.items = addRouteItemRecursion(
        obj.items ?? [],
        item,
        path,
        upgradeIndex,
...
function sidebarSort(sidebar: ArticleTree[], folderTop: boolean = true) {
  ...
  if (folderTop) {
    // 分别找出直接的文件和嵌套文件夹
    const files = articleTreeSort(
      sidebar.filter((item) => {
        return !item.items || item.items.length === 0;
      }),
    );
    const folders = articleTreeSort(
      sidebar.filter((item) => {
        return item.items && item.items.length > 0;
      }),
    );
...
function isTagAliasOfTag(srcTag: string, targetTag: string) {
  return (
    srcTag === targetTag || srcTag.toUpperCase() === targetTag.toUpperCase()
  );
}

function findTagAlias(
  tag: string,
  docsMetadata: DocsMetadata,
  aliasMapping: DocsTagsAlias[],
) {
  ...
  docsMetadata.tags.forEach((item) => {
    ...
    if (isTagAliasOfTag(item.name, tag)) {
      // 如果有记录的 tag.name 是当前 tag 的别名
  ...
  for (const tag of tags) {
    ...
    docsMetadata.tags.forEach((item, index) => {
      aliases.forEach((alias) => {
        if (
          item.name === alias &&
          !docsMetadata.tags[index].appearedInDocs.includes(doc)
        )
...
async function processDocs(docs: string[], docsMetadata: DocsMetadata) {
  ...
  docsMetadata.docs = docs.map((docPath) => {
    ...
    if (Array.isArray(parsedPageContent.data.tags)) {
      ...
      tagsToBeProcessed.push({
        doc: docPath,
        tags: parsedPageContent.data.tags,
      });
    }
    ...
    if (!found) {
      ...
      if (
        found.hashes.sha256.content === tempSha256Hash &&
        !found.hashes.sha256.contentDiff
      )
        return found;
  ...
  await Promise.all(
    tagsToBeProcessed.map(async ({ doc, tags }) => {
...
async function run() {
  ...
  await fs.writeJSON(join(DIR_VITEPRESS, "docsMetadata.json"), docsMetadata, {
    spaces: 2,
  });
}
